...
const content = {
    th: {
        ...
        resources: {
            ...
            faqs: {
                title: 'คำถามพบบ่อย',
                description: 'ใช้กับคำถามที่มีคำตอบชัดเจนและถูกถามซ้ำ เช่น การจองคิว การชำระเงิน การเลื่อนนัด หรือการเตรียมตัว',
        ...
        <div className="inline-flex rounded-lg border border-slate-300 bg-white p-1" role="group" aria-label={t.languageLabel}>
            {(['th', 'en'] as const).map((value) => (
                <button
                    ...
                    className={`rounded-md px-3 py-1.5 text-sm font-medium transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-600 ${
                        language === value ? 'bg-green-700 text-white' : 'text-slate-600 hover:bg-slate-100'
                    }`}
                >
        ...
        <AdminLayout title={t.pageTitle} actions={languageSwitcher}>
            ...
            <section className="rounded-xl bg-slate-900 px-5 py-6 text-white sm:px-7">
                <div className="flex max-w-3xl items-start gap-4">
                    <BookOpenCheck className="mt-1 h-7 w-7 shrink-0 text-green-400" />
                    <div>
                        ...
                        <div className="mt-4 flex items-start gap-2 rounded-lg bg-white/10 px-3.5 py-3 text-sm leading-6 text-slate-200">
                            <Info className="mt-0.5 h-4 w-4 shrink-0 text-green-300" />
                            <p>{t.important}</p>
                        </div>
                    </div>
                </div>
                    ...
                    {resourceDefinitions.map((resource, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <Link
                                    href={resource.href}
                                    className="inline-flex items-center gap-1.5 text-sm font-medium text-green-700 hover:text-green-800 hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-600"
                                >
                                    {t.openForm}
            ...
            <section className="mt-8 flex flex-col gap-4 rounded-xl border border-slate-200 bg-white px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                <div className="flex max-w-3xl items-start gap-3">
                    <FileSpreadsheet className="mt-0.5 h-5 w-5 shrink-0 text-green-700" />
                    <div>
                ...
                <Link
                    ...
                    className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-green-700 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-green-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-600"
                >
                    {t.goImport}
                    <ArrowRight className="h-4 w-4" />
                </Link>
            ...
            <section className="mt-8">
                <div className="flex items-start gap-3">
                    <Database className="mt-0.5 h-5 w-5 shrink-0 text-green-700" />
                    <div>
            ...
            <section className="mt-8 rounded-xl bg-green-50 px-5 py-6 sm:px-6">
                <div className="flex items-center gap-2">
                    <Bot className="h-5 w-5 text-green-800" />
                    <h2 className="font-semibold text-green-950">{t.checklistTitle}</h2>
                </div>
                <ul className="mt-4 grid gap-3 md:grid-cols-2">
                    {t.checklist.map((item) => (
                        <li key={item} className="flex items-start gap-2 text-sm leading-6 text-green-950">
                ...
                <Link
                    ...
                    className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-green-800 hover:underline focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-green-700"
                >
