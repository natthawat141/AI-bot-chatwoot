...
A low-cost, single-deploy **Laravel modular monolith** that manages the knowledge base for a
LINE bot: service packages, FAQs, and free-form knowledge entries. It exposes
a small authenticated **read API** for the LINE bot service (`line-bot-service`) to consume.
...
only outward boundary is the authenticated compact **read API** consumed by `line-bot-service`.
...
The seeder creates the admin from the `ADMIN_*` env values (skipped with a warning if
`ADMIN_PASSWORD` is blank) and inserts verified **W+ Medic Clinic** answer content.
...
- **Answer analytics** — latest LINE questions and bot answers, daily/total counts, success rate, and response type without storing raw LINE user IDs
...
## 6. Knowledge read API (for line-bot-service)
...
```powershell
php artisan api-token:issue "line-bot-service" --expires=365
